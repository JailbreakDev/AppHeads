...
-(NSBundle *)localizationBundle {
	return [NSBundle bundleWithPath:@"/Library/PreferenceBundles/AppHeadsSettings.bundle"];
}
...
-(NSArray *)localizedSpecifiersForSpecifiers:(NSArray *)specifiers inTable:(NSString *)table {
    ...
    for(PSSpecifier *spec in specifiers) {
        ...
        if (footerText) {
            if ([footerText isEqualToString:@"DISPLAY_MODE_DESCRIPTION"]) {
                [spec setProperty:[NSString stringWithFormat:[self localizedStringForKey:footerText inTable:table],[self localizedStringForKey:@"REQUIRES_RESPRING" inTable:table]] forKey:@"footerText"];
            } else {
